...
int check_cache_data_hit(void* addr, char type) {
    ...
    int block_offset = (int)(address % DEFAULT_CACHE_BLOCK_SIZE_BYTE);
    int set_index = (int)((address / DEFAULT_CACHE_BLOCK_SIZE_BYTE) % CACHE_SET_SIZE);
...
int find_entry_index_in_set(int cache_index) {
    int entry_index = -1;
    int oldest_timestamp = global_timestamp;
    ...
    return entry_index; // Return LRU entry index if no empty slot
}

int access_memory(void* addr, char type) {
    uintptr_t address = (uintptr_t)addr;
    int word_address = (int)(address / WORD_SIZE_BYTE);
    int block_offset = (int)(address % DEFAULT_CACHE_BLOCK_SIZE_BYTE);
    int set_index = (int)((address / DEFAULT_CACHE_BLOCK_SIZE_BYTE) % CACHE_SET_SIZE);
    int tag = (int)((address / DEFAULT_CACHE_BLOCK_SIZE_BYTE) / CACHE_SET_SIZE);
